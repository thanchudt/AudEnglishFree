...
import com.dangle1107.audenglishlibrary.MyBundle;
import com.dangle1107.audenglishlibrary.Sentence;
...
public class Player_20140901Activity extends Activity implements OnCompletionListener, SeekBar.OnSeekBarChangeListener{
	...
	private LessonDataSource _audioDataSource;
	private List<Lesson> audioList;
	private SentenceDataSource _lyricDataSource;
	private List<Sentence> lyricList = null;
	 ...
	 // Add the AdView to the view hierarchy. The view will have no size
		...
		btnNext.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if(myBundle.currentSongIndex < (audioList.size() - 1)){
		...
		btnPrevious.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if(myBundle.currentSongIndex > 0){					
					...
					// play last song
					myBundle.currentSongIndex = audioList.size() - 1;									
				}
				isUpdateLyric = true;				
				playSong(myBundle.currentSongIndex);
			}
    ...
    protected void onActivityResult(int requestCode,
        ...
        if(resultCode == 100){
        	lyricList = null;
        	myBundle.currentSongIndex = data.getExtras().getInt(PlayListActivity.PLAYLIST_EXTRA_MESSAGE_LESSON_INDEX);
        	 // play selected song
            playSong(myBundle.currentSongIndex);
       }
    }
	...
	public void  playSong(int songIndex){
		...
		try {			
			...
			resetPlayer();
			if(songIndex >= audioList.size())
				return;
        	//int resID=getResources().getIdentifier(songList.get(songIndex).getIdentification(), "raw", getPackageName());
			_lyricDataSource = new SentenceDataSource(this);
			_lyricDataSource.open();
			lyricList = _lyricDataSource.getAllSentences(audioList.get(songIndex).getId());
			_lyricDataSource.close();
			Collections.sort(lyricList, new SentenceComparator());
        	Uri path = Uri.parse("android.resource://" + getPackageName() + "/raw/" + audioList.get(songIndex).getIdentification());
			...
			String songTitle = audioList.get(songIndex).getName();
	...
	void showLyric(int lyricIndex){
		if(myBundle.currentSongIndex >= audioList.size() || lyricList == null)
			return;
		if(lyricIndex >= 0 && lyricIndex < lyricList.size())
		{		
			//lyricLabel.setText(lyricList.get(lyricIndex).getLyric());
			Sentence lyric = lyricList.get(lyricIndex);
			String path= "@drawable/" + audioList.get(myBundle.currentSongIndex).getIdentification() + String.format("%05d", lyric.getPosition());			
			try {
	...
	public void onCompletion(MediaPlayer arg0) {
		...
		if(myBundle.isRepeat){			
			...
			myBundle.currentSongIndex = rand.nextInt((audioList.size() - 1) - 0 + 1) + 0;			
		} else{
